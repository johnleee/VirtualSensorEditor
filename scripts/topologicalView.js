            ...
            function update(source) {
                ...
                var nodeEnter = node.enter().append("svg:g") // Enter any new nodes at the parent's previous position.
                    ...
                    .on("click", function (d) {
                        ...
                        if (d.type == "Sensor") {
                            if (d.children) {
                                sn_visualization.timeseriesView.remove(d.d_uri, d.s_id);
                            }
                            else {
                                sn_visualization.timeseriesView.insert(d.d_uri, d.s_id, d.d_name, d.name);
                            }
                ...
                nodeEnter.append("svg:text")
                    ...
                    .attr("data-d_uri", function (d) {
                        return d.d_uri || "";
                    })
                    .attr("data-s_id", function (d) {
                        return d.s_id || "";
                    })
                    .attr("data-d_name", function (d) {
                        return d.d_name || "";
                    })
    ...
    return {
        ...
        updateStatus: function (data) {
            ...
            for (var index in data) {
                var
                    offset = now.getTime() - data[index].timestamp,
                    targetCircle = $('#topologicalView svg text[data-d_uri="' + data[index].device_id + '"]').parent().find('circle');

                if (offset > 3 * 60 * 1000) {
                    statusColorTable[data[index].device_id] = "red";
                }
                else if (offset > 15 * 1000) {
                    statusColorTable[data[index].device_id] = "khaki";
                }
                else {
                    statusColorTable[data[index].device_id] = "steelBlue";
                }
                targetCircle.css('fill', statusColorTable[data[index].device_id]);
            }
        }
    };
